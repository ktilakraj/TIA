...
#import "ItinararyTableViewCell.h"

@interface ItineraryViewController ()<UITextFieldDelegate,UITableViewDelegate,UITableViewDataSource>
...
[self.tableView registerNib:[UINib nibWithNibName:@"ItinararyTableViewCell" bundle:nil] forCellReuseIdentifier:@"checkListCell"];
...
-(void)updateheader
{
    if (_selctedCheckListOrToDo == 3) {
        
        _strKey=@"itinrary";
        _lblTitle.text=@"ITINERARY";
        _lblFrmToDate.text=@"";
...
-(void)pickerinitlization
{
    ...
    self.txtChecklist.inputView = _datePicker;
    // Set UITextfield's inputAccessoryView as UIToolbar
    //self.txtChecklist.inputAccessoryView = _datePickerToolbar;
...
-(void)updateTextFields
{
    ...
    if (_txtSelected == 101) {
        ...
        self.txtChecklist.text = [df stringFromDate:selectedDate];
...
//                f.origin.y = -(keyboardSize.height+40);
//                self.view.frame = f;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    ItinararyTableViewCell *cell=[tableView dequeueReusableCellWithIdentifier:@"checkListCell"];
    /*
     if (!cell) {
    ...
    cell.lbldate.text=[dict valueForKey:@"seletectedDate"];
...
- (IBAction)btnAddClick:(id)sender {
    ...
    if (_txtChecklist.text.length > 0 && _txtcost.text.length > 0) {
       
        NSMutableDictionary *dict=[[NSMutableDictionary alloc] init];
        [dict setObject:self.txtcost.text forKey:@"name"];
        [dict setObject:self.txtChecklist.text forKey:@"seletectedDate"];
